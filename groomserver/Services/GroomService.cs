...
public class GroomService : Groom.GroomBase
{
    ...
    public override async Task<NewStreamStatus> SendNewsFlash(IAsyncStreamReader<NewsFlash> newsStream, ServerCallContext context)
    {
        while( await newsStream.MoveNext()) {
            var news = newsStream.Current;
            MessagesQueue.AddNewsToQueue(news);
            UsersQueues.AddNewsFlashForUsers(news);
            Console.WriteLine($"News flash: {news.NewsItem}");
        }

        return new NewStreamStatus { Succes = true };
    }
